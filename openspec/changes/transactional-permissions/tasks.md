...
- [ ] 1.1.1 添加 `PermissionCheckResult` 类型定义
  - 包含权限分组 (必需/可选)
  - 包含风险评估 (low/medium/high)
  - 包含推荐策略
- [ ] 1.1.2 更新 `PermissionManager.checkPermissions()` 实现
  - 支持权限分组检查
  - 计算风险等级
  - 生成推荐策略
- [ ] 1.1.3 添加单元测试
  ...
  - 测试风险计算
  - 测试永久拒绝检测
- [ ] 1.1.4 更新 IPC handler
  - 添加 `permission:checkPermissions` handler
  - 处理权限分组参数
- [ ] 1.1.5 更新 preload 类型定义
  - 在 `ipc.ts` 中添加 `checkPermissions` API
  - 包含完整的类型定义

### 1.2 创建 FeaturePermissionDialog 组件
- [ ] 1.2.1 创建 `FeaturePermissionDialog.tsx` 组件
  - 显示功能名称和描述
...
- [ ] 1.3.1 添加 `preCheckPermissions()` 方法
  - 接受权限数组参数
  - 返回 `PermissionCheckResult`
- [ ] 1.3.2 添加 `requestFeaturePermissions()` 方法
  - 接受功能名称、权限数组、原因
  - 显示 FeaturePermissionDialog
  - 返回授权结果
- [ ] 1.3.3 更新插件上下文实现
...
- [ ] 2.1.1 创建 `transaction-executor.ts` 文件
  - 定义 `Transaction` 接口
  - 定义 `TransactionStep` 接口
  - 定义 `TransactionResult` 接口
- [ ] 2.1.2 实现核心 `execute()` 方法
  - 遍历执行所有步骤
  - 错误捕获和处理
  - 自动触发回滚
- [ ] 2.1.3 实现步骤超时机制
...
### 2.2 集成进度报告
- [ ] 2.2.1 在 TransactionExecutor 中集成 ProgressReporter
  - 执行开始: 调用 `start()`
  - 每个步骤: 调用 `update()`
  - 执行成功: 调用 `complete()`
  - 执行失败: 调用 `fail()`
...
### 2.3 实现事务日志
- [ ] 2.3.1 创建 `transaction-logger.ts`
...
- [ ] 2.4.1 添加 `executeTransaction()` 方法
  - 接受 `Transaction` 参数
  - 返回 `TransactionResult`
- [ ] 2.4.2 实现 `TransactionBuilder` 类
  - 流式 API 设计
  - `addStep()` 方法
  - `build()` 方法
- [ ] 2.4.3 添加 `createTransaction()` 工厂方法
