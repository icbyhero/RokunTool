...
定义插件操作进度反馈机制,为长时间运行的插件操作提供统一的用户反馈接口,改善用户体验。
...
- **plugin-system**: 插件生命周期管理、插件 API
- **wechat-multi-instance**: 微信分身创建、更新、批量操作

### Dependency Graph

```
plugin-operation-progress
  ├─→ plugin-system (依赖插件 API 框架)
  └─→ wechat-multi-instance (首个应用此能力的插件)
```
